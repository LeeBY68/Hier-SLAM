...
def transformed_params2rendervar_semantic(params, transformed_gaussians):
    
    """
    N: num of 3d gaussian
    means3D:          [N, 3]                    camera coordinate
    rgb_colors:       [N, 3]                    color for 3d GS
    unnorm_rotations: [N, 4]                    R for 3d GS
    logit_opacities:  [N, 1]                    alpha for 3d GS
    log_scales:       [N, 1]                    scale for 3d GS
    semantic:         [N, num_classes]          semantic for 3d GS
    cam_unnorm_rots:  [1, 4, num_frame]         frame's pose R
    cam_trans:        [1, 3, num_frame]         frame's pose t
    """

    rendervar = {
        'means3D': transformed_gaussians['means3D'],
        'colors_precomp': params['rgb_colors'],
        'rotations': F.normalize(params['unnorm_rotations']),               # also can be transformed 'transformed_gaussians'
        'opacities': torch.sigmoid(params['logit_opacities']),
...
def get_depth_and_silhouette(pts_3D, w2c):
    ...
    return depth_silhouette  # [z, 1, z^2]
