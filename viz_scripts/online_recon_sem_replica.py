...
from utils.recon_helpers import setup_camera, setup_camera_newtest
from utils.slam_helpers import get_depth_and_silhouette
from utils.slam_external import build_rotation


flag_mlp = True
label_colour_map = label_colormap(550)


def semantic_labelmapvis(im_semantic_in):
    im_semantic_show = im_semantic_in.detach().cpu().numpy().copy()
    im_semantic_labelmap = label_colour_map[im_semantic_show]
    im_semantic_labelmap = im_semantic_labelmap.astype(np.uint8) 
    im_semantic_labelmap = np.squeeze(im_semantic_labelmap)
    im_semantic_labelmap = torch.from_numpy(im_semantic_labelmap).cuda()
    im_semantic_labelmap = im_semantic_labelmap.permute(2,0,1)

    return im_semantic_labelmap


def process_semantic(im_semantic_in, MLP_func):
    logits = MLP_func(im_semantic_in)
    im_semantic_out = torch.argmax(logits, dim=0)
...
def get_rendervars_semantic(params, w2c, curr_timestep):
    ...
    rendervar = {
        ...
        'opacities': torch.sigmoid(selected_params['logit_opacities']),
        'scales': torch.exp(log_scales),
        'semantics_precomp': params['semantic'],
        'means2D': torch.zeros_like(selected_params['means3D'], device="cuda")
    }
...
def render_semantic(w2c, k, timestep_data, cfg):
    with torch.no_grad():
        cam = setup_camera_newtest(cfg['viz_w'], cfg['viz_h'], k, w2c, cfg['viz_near'], cfg['viz_far'])
        white_bg_cam = Camera(
            image_height=cam.image_height,
            image_width=cam.image_width,
            tanfovx=cam.tanfovx,
        ...
        im, radius, im_semantic, rendered_depth, _, rendered_final_opcity \
        = Renderer_semantic(raster_settings=white_bg_cam)(**timestep_data)
        sil = rendered_final_opcity
        return im, rendered_depth, im_semantic, sil
...
def visualize(scene_path, cfg, mlp_func=None, flag_save=False):
    ...
    scene_data = get_rendervars_semantic(params, first_frame_w2c, curr_timestep=0)
    im, depth, im_semantic, sil = render_semantic(first_frame_w2c, k, scene_data, cfg)
    if flag_show_semantic:
        im_semantic = process_semantic(im_semantic, mlp_func)
    ...
    init_pts, init_cols = rgbd2pcd(im, depth, first_frame_w2c, k, cfg)      # Get 1st frame 3d pts
    ...
    while curr_timestep < (num_timesteps-1) or not cfg['enter_interactive_post_online']:
        ...
        scene_data = get_rendervars_semantic(params, view_w2c, curr_timestep=curr_timestep)
        if cfg['render_mode'] == 'centers':
            ...
            im, depth, im_semantic, sil = render_semantic(view_w2c, k, scene_data, cfg)
            if flag_show_semantic:
                im_semantic = process_semantic(im_semantic, mlp_func)
                im_semantic_labelmap = semantic_labelmapvis(im_semantic)
                im_semantic_labelmap = im_semantic_labelmap.to(im.dtype)
        ...
        if flag_save:
            vis.capture_screen_image(save_path+"{}.png".format(curr_timestep))
...
if __name__ == "__main__":
    ...
    # replica
    num_semantic_dim = 26
    num_semantic_class = 102
    # scannet-ablation large
    # num_semantic_dim = 74
    # num_semantic_class = 550
    if flag_show_semantic and flag_mlp:
        MLP_func = torch.nn.Conv2d(num_semantic_dim, num_semantic_class, kernel_size=1)
    ...
    if flag_show_semantic and flag_mlp:
        visualize(scene_path, viz_cfg, mlp_func = MLP_func, flag_save=False)
    else:
        visualize(scene_path, viz_cfg, flag_save=False)
