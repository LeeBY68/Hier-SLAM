...
def visualize(scene_path, cfg):
    ...
    vis.create_window(width=int(cfg['viz_w'] * cfg['view_scale']), 
                      ...
                      visible=True)
    # print(w2c.shape, k.shape)       # [4,4]*16, [3,3]
    im, depth, sil = render(w2c, k, scene_data, scene_depth_data, cfg)      # change
    init_pts, init_cols = rgbd2pcd(im, depth, w2c, k, cfg)                  # color1
    ...
    while True:
        ...
        if cfg['render_mode'] == 'centers':
            ...
            pts, cols = rgbd2pcd(im, depth, w2c, k, cfg)        # color2
